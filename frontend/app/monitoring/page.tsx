...
interface StressResult {
  deal_id: string; company: string; sector: string; loan_amount: number;
  current_rating: string; stressed_rating: string;
  coverage_breach: boolean; leverage_breach: boolean; notches: number;
}
function calcStressResults(portfolio: Deal[], scenario: string): StressResult[] {
  return portfolio.map((deal): StressResult => {
    ...
    return { deal_id: deal.deal_id, company: deal.company, sector: deal.sector, loan_amount: deal.loan_amount,
      current_rating: deal.internal_rating, stressed_rating: migrateRating(deal.internal_rating, notches),
    ...
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="text-muted text-xs font-mono">Monitoring agents active</span>
        </div>
      </div>
        ...
        <div className="space-y-5">
          <div className="flex items-center gap-4">
            ...
            {deal && (
              <div className="flex items-center gap-4 ml-2">
                <span className="text-muted text-xs font-mono">Risk: <span style={{ color: getRiskColor(deal.risk_score) }} className="font-bold">{deal.risk_score}</span></span>
                <span className="text-muted text-xs font-mono">Sector stress: <span style={{ color: getRiskColor(deal.sector_stress_score) }} className="font-bold">{deal.sector_stress_score}</span></span>
          ...
          <div className="glass rounded-lg overflow-hidden">
            <div className="flex items-center justify-between px-5 py-3 border-b border-white/[0.06]">
              <div>
                ...
                <p className="text-muted text-[10px] uppercase tracking-wider mt-0.5">News intelligence · Sentiment scoring · Early warning signals · Alt-data</p>
              </div>
            ...
            {/* Agent pipeline — visible while running */}
                ...
                {monitorAgents.map((a, i) => (
                  <div key={i} className={cn("flex items-center gap-2 px-3 py-2 rounded-md text-xs transition-all duration-300",
                    ...
                    <span className={cn("text-lg leading-none",
                      a.status === "complete" ? "text-success" :
                      a.status === "running"  ? "text-accent" : "text-muted/40"
                    )}>
                      {a.status === "complete" ? "✓" : a.status === "running" ? "◉" : "○"}
                    </span>
                    <span className={cn("font-mono",
                      a.status === "complete" ? "text-success" :
                      a.status === "running"  ? "text-accent" : "text-muted/50"
                    )}>{a.name}</span>
                  </div>
                ))}
              </div>
            ...
            <div className="p-5 grid grid-cols-2 gap-5">
              <div className="space-y-4">
                ...
                {warningFlags.length > 0 && (
                  <div className="space-y-1">
                    <p className="text-primary text-xs font-semibold mb-2">Warning Flags</p>
                    {warningFlags.map((f: any, i: number) => (
                      <div key={i} className={cn("rounded px-3 py-2 text-xs border-l-2",
                        ...
                        f.severity === "HIGH"     ? "border-warning bg-warning/5 text-warning" :
                                                    "border-muted bg-white/[0.02] text-muted"
                      )}>
              ...
              <div className="space-y-4">
                <div className="bg-black/40 border border-white/[0.06] rounded-lg p-4 space-y-2">
                  <p className="text-primary text-xs font-semibold">Deal Metrics</p>
                  {[
                    ["Sector", deal?.sector],
                    ...
                    <div key={k} className="flex justify-between">
                      ...
                      <p className="text-primary text-xs font-mono">{v}</p>
                    </div>
                  ))}
                </div>
          ...
          <div className="glass rounded-lg overflow-hidden">
            ...
            {(qRunning || (quarterlyAgents.length > 0 && qSummary)) && (
              <div className="px-5 pt-4 pb-2 border-b border-white/[0.04] grid grid-cols-4 gap-2">
      ...
      {tab === "stress" && (
        <div className="space-y-5">
          <div className="glass rounded-lg p-5">
            ...
            <p className="text-muted text-xs mb-4">Apply macro scenarios across all {state.portfolio.length} loans to identify covenant breaches and rating migrations.</p>
            <div className="grid grid-cols-2 gap-3 mb-4">
              {STRESS_SCENARIOS.map(s => (
                <button key={s.id} onClick={() => setScenario(s.id)}
                  className={cn("text-left p-3 rounded-lg border transition-all duration-150",
            ...
            <>
              ...
              <div className="glass rounded-lg overflow-hidden animate-fade-up delay-300">
                <div className="px-5 py-3 border-b border-white/[0.06] flex items-center justify-between">
                  <p className="text-primary text-sm font-semibold">Impacted Loans — {STRESS_SCENARIOS.find(s => s.id === scenario)?.label}</p>
                ...
                <table className="w-full text-xs">
                  ...
                  <tbody>
                    {stressResults.map((r, i) => (
                      <tr key={r.deal_id} className={cn("border-b border-white/[0.04] last:border-0 animate-fade-up", i % 2 === 1 && "bg-white/[0.02]")} style={{ animationDelay: `${i * 0.04}s` }}>
                        ...
                        <td className="px-4 py-2.5"><span className="font-mono font-bold" style={{ color: getRiskColor(50 + r.notches * 15) }}>{r.stressed_rating}</span><span className="text-danger ml-1 text-[10px]">▼{r.notches}</span></td>

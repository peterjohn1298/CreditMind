...
export default function Alerts() {
  const { state, dispatch, triggerRefresh, apiOnline } = useCredit();
    ...
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between gap-4 flex-wrap">
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            onClick={apiOnline ? triggerRefresh : undefined}
            disabled={state.isRefreshing || apiOnline === false}
            title={apiOnline === false ? "API offline — set NEXT_PUBLIC_API_URL in Vercel to enable live refresh" : undefined}
            className={cn("flex items-center gap-2 px-4 py-1.5 rounded-md text-sm font-medium border transition-all",
              state.isRefreshing ? "border-white/[0.08] text-muted cursor-not-allowed" :
              apiOnline === false ? "border-white/[0.08] text-muted/40 cursor-not-allowed" :
              "border-accent text-accent hover:bg-accent hover:text-white"
            )}>
            <RefreshCw size={13} className={state.isRefreshing ? "animate-spin" : ""} />
            {state.isRefreshing ? "Agents running…" : apiOnline === false ? "Refresh (offline)" : "Refresh"}
          </button>
        </div>
      </div>
      ...
      {tab === "sector" && (
        <div className="space-y-3 max-w-2xl">
          {apiOnline === false && (
            <div className="glass rounded-lg p-4 border border-warning/20 flex items-start gap-3">
              <RefreshCw size={13} className="text-warning shrink-0 mt-0.5" />
              <div>
                <p className="text-warning text-xs font-semibold">Showing demo sector alerts</p>
                <p className="text-muted text-[11px] mt-0.5 leading-relaxed">
                  Live sector alerts are generated by portfolio scan agents. Set <span className="font-mono text-white/50">NEXT_PUBLIC_API_URL</span> in Vercel and redeploy to enable real-time sector monitoring.
                </p>
              </div>
            </div>
          )}
